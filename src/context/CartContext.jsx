...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = async (menuItem, quantity = 1) => {
    try {
      const response = await fetch('https://restaurant-reservation-system-backend-qq29.onrender.com/api/cart/add', {
        method: 'POST',
        headers: {
  ...
  const removeFromCart = async (menuItemId, quantity = 1) => {
    try {
      const response = await fetch(`https://restaurant-reservation-system-backend-qq29.onrender.com/api/cart/remove/${menuItemId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': localStorage.getItem('token'),
  ...
  const fetchCart = useCallback(async () => {
    try {
      const response = await fetch('https://restaurant-reservation-system-backend-qq29.onrender.com/api/cart', {
        headers: {
          'x-auth-token': localStorage.getItem('token'),
        },
      });
