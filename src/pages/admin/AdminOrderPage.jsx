...
const AdminOrderPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch("https://restaurant-reservation-system-backend-qq29.onrender.com/api/order/all", {
        headers: {
          "x-auth-token": token,
        },
      });
  ...
  const handleStatusChange = async (orderId) => {
    try {
      const response = await fetch(
        `https://restaurant-reservation-system-backend-qq29.onrender.com/api/order/${orderId}/status`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://restaurant-reservation-system-backend-qq29.onrender.com/api/order/${id}`, {
        method: "DELETE",
        headers: {
          "x-auth-token": token,
        },
