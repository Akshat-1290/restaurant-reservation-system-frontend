  ...
  useEffect(() => {
   if(selectedDate && selectedTime){
    const fetchBookedTables = async () => {
      const response = await api.get(`https://restaurant-reservation-system-backend-qq29.onrender.com/api/reservations/booked?date=${selectedDate}&time=${selectedTime}`)
...
const handleReservation = async () => {
  if (selectedTable && selectedDate && selectedTime) {
    setLoading(true)
    try {
      const response = await api.post('https://restaurant-reservation-system-backend-qq29.onrender.com/api/reservations/create-checkout-session', {
